...
int getCars(void* NotUsed, int argc, char** argv, char** azColName);
...
int getAdmin(void* NotUsed, int resultNum, char** values, char** colNames);
int selectData(const char* dir, string query, int code);
int updateData(const char* dir, string query);

struct driverIsBlockedException : public exception
{
public:
    const char* what() const throw ()
    {
        return "You have been blocked by admin. You can not take orders";
    }
};

struct passengerIsBlockedException : public exception
{
public:
    const char* what() const throw ()
    {
        return "You have been blocked by admin. You can not order orders";
    }
};
...
public:
    ...
    Car(string model, string color, string carType, string number, string status, int latitude, int longitude, string validation) {
        this->model = model;
        this->color = color;
    ...
    Car(string model, string color, string carType, string number, string validation) {
        this->model = model;
        this->color = color;
        this->carType = carType;
        this->number = number;
...
public:
    ...
    ComfortCar(string model, string color, string carType, string number, string status, int latitude, int longitude, string validation) :
        Car(model, color, carType, number, status, latitude, longitude, validation)
    {
...
public:
    BussnessCar(string model, string color, string carType, string number, string status, int latitude, int longitude, string validation) :
        Car(model, color, carType, number, status, latitude, longitude, validation)
    {
...
public:
    ...
    string name;
    string password;
    vector<Car> carsForValidation;
    Admin(int id, string name, string password) {
        this->id = id;
        this->name = name;
        this->password = password;
        this->carsForValidation = vector<Car>();
    }
    Admin() {
    }
};
...
public:
    ...
    Passenger(string name, string address, string payment1, string payment2, string password, vector<Ride> history, int rating, int id, bool isBlocked) {
        this->name = name;
        this->address = address;
        this->payment1 = payment1;
        this->payment2 = payment2;
...
public:
    ...
    Driver(int id, int rating, string name, string password, vector<Ride> history, Car car, string status, string isBlocked) {
        this->id = id;
        this->rating = rating;
        this->name = name;
        this->password = password;
    ...
    Driver(int id, int rating, string name, string password, vector<Ride> history, string status, string isBlocked) {
        this->id = id;
        this->rating = rating;
        this->name = name;
        this->password = password;
...
public:
    ...
    void logIn(string name, string password) {
        ...
        // admin with given name and password if found
        // fills variable admin with data of given admin

        string sql_query = "SELECT * FROM admin WHERE name = '" + name + "' AND password = '" + password + "';";
        selectData("WEndex_Taxi_BD.db", sql_query, 6);
        admin = tmpAdmin;
        if (admin.name != "") {
            cout << endl << "Authorization was successful" << endl;
            cout << "Hello, " + admin.name << endl;
        }
    ...
    void logInStory() {
        ...
        // admin with given name and password if found
        // fills variable admin with data of given admin
        cout << endl << "-----------Admin John Wick logging in to the system-----------" << endl;

        string sql_query = "SELECT * FROM admin WHERE name = 'John Wick' AND password = 'UKilledMyDog';";
        selectData("WEndex_Taxi_BD.db", sql_query, 6);
        admin = tmpAdmin;
        if (admin.name != "") {
            cout << endl << "Authorization was successful" << endl;
    ...
    void blockPassenger() {
        //This method blocks given passenger
        cin.ignore();
        string passengerName;
        cout << "Enter name of passenger to block" << endl;
        getline(cin, passengerName);
        string sql_query = "UPDATE passenger SET isBlocked = 'true' WHERE name = '" + passengerName + "';";
        updateData("WEndex_Taxi_BD.db", sql_query);
        cout << "Passenger " << passengerName << " have been blocked" << endl;
    }
    void blockPassengerStory() {
        //This method blocks given passenger
        cout << endl << "-----------Admin " << admin.name << " blocking passenger-----------" << endl;
        string sql_query = "UPDATE passenger SET isBlocked = 'true' WHERE name = 'Khabib Khays';";
        updateData("WEndex_Taxi_BD.db", sql_query);
        cout << "Passenger Khabib Khays have been blocked" << endl;
    }
    void blockDriver() {
        //This method blocks given driver
        cin.ignore();
        string driverName;
        cout << "Enter name of driver to block" << endl;
        getline(cin, driverName);

        string sql_query = "UPDATE driver SET isBlocked = 'true' WHERE name = '" + driverName + "';";
        updateData("WEndex_Taxi_BD.db", sql_query);
        cout << "Driver " << driverName << " have been blocked" << endl;
    }
    void blockDriverStory() {
        //This method blocks given driver
        cout << endl << "-----------Admin " << admin.name << " blocking driver-----------" << endl;
        string sql_query = "UPDATE driver SET isBlocked = 'true' WHERE name = 'Vagif Khalilov';";
        updateData("WEndex_Taxi_BD.db", sql_query);
        cout << "Driver Vagif Khalilov have been blocked" << endl;
    }
    void unblockPassenger() {
        //This method unblocks given passenger
        cin.ignore();
        string passengerName;
        cout << "Enter name of passenger to unblock" << endl;
        getline(cin, passengerName);

        string sql_query = "UPDATE passenger SET isBlocked = 'false' WHERE name = '" + passengerName + "';";
        updateData("WEndex_Taxi_BD.db", sql_query);
        cout << "Passenger " << passengerName << " have been unblocked" << endl;
    }
    void unblockDriver() {
        //This method unblocks given driver
        cin.ignore();
        string driverName;
        cout << "Enter name of driver to unblock" << endl;
        getline(cin, driverName);

        string sql_query = "UPDATE driver SET isBlocked = 'false' WHERE name = '" + driverName + "';";
        updateData("WEndex_Taxi_BD.db", sql_query);
        cout << "Driver " << driverName << " have been unblocked" << endl;
    }
    void unblockDriverStory() {
        //This method unblocks given driver
        cout << endl << "-----------Admin " << admin.name << " unblocking driver-----------" << endl;
        string sql_query = "UPDATE driver SET isBlocked = 'false' WHERE name = 'Vagif Khalilov';";
        updateData("WEndex_Taxi_BD.db", sql_query);
        cout << "Driver Vagif Khalilov have been unblocked" << endl;
    ...
    void validateCar() {
        ...
        sql_query = "SELECT * FROM cars WHERE validation = 'waiting';";

        selectData("WEndex_Taxi_BD.db", sql_query, 7);
        admin.carsForValidation = tmpCars;
        tmpCars = vector<Car>();
        cin.ignore();
        int code = 0;
        for (int i = 0; i < admin.carsForValidation.size(); i++)
        {
            cout << endl;
            cout << "Validation Car Number " << i + 1 << " :" << endl << endl;
            cout << "Model: " << admin.carsForValidation[i].model << endl;
            cout << "Color: " << admin.carsForValidation[i].color << endl;
            cout << "Car Type: " << admin.carsForValidation[i].carType << endl;
            cout << "Car number:" << admin.carsForValidation[i].number << endl;
            cout << "Validation: " << admin.carsForValidation[i].validation << endl;
            cout << endl;
            ...
            while (code == 0)
            {
                ...
                if (code != 1 && code != 2){
                    ...
                    string sql_query = "UPDATE cars SET validation = 'validated' WHERE number = '" +
                        admin.carsForValidation[i].number + "';";
                    updateData("WEndex_Taxi_BD.db", sql_query);
                }
                else if (code == 2) {
                    string sql_query = "UPDATE cars SET validation = 'not validated' WHERE number = '" +
                        admin.carsForValidation[i].number + "';";
                    updateData("WEndex_Taxi_BD.db", sql_query);
                }
            }
    ...
    void validateCarStory() {
        string sql_query;
        cout << endl << "-----------Admin " << admin.name << " checks cars for validation-----------" << endl;
    ...
    void orderTaxi() {
        try
        {
            if (passenger.isBlocked)
                throw passengerIsBlockedException();
        ...
        catch(passengerIsBlockedException e) {
            cout << e.what() << endl;
        }
    }
...
public:
    ...
    void getCar() {
        ...
        if (carType == "Economy" || carType == "ComfortPlus")
            driver.car = tmpCar;
        ...
        if (driver.bussnesCar.number != "" && carType == "Business")
            sql_query = "UPDATE cars SET status = 'busy' WHERE number = '" + driver.bussnesCar.number + "';";
        if (driver.comfortCar.number != "" && carType == "Comfort")
            sql_query = "UPDATE cars SET status = 'busy' WHERE number = '" + driver.comfortCar.number + "';";
        if (driver.car.number != "" && carType == "Economy" || carType == "ComfortPlus")
            sql_query = "UPDATE cars SET status = 'busy' WHERE number = '" + driver.car.number + "';";

        updateData("WEndex_Taxi_BD.db", sql_query);

        if (driver.bussnesCar.number != "" && carType == "Business")
            sql_query = "UPDATE cars SET validation = 'waiting' WHERE number = '" + driver.bussnesCar.number + "';";
        if (driver.comfortCar.number != "" && carType == "Comfort")
            sql_query = "UPDATE cars SET validation = 'waiting' WHERE number = '" + driver.comfortCar.number + "';";
        if (driver.car.number != "" && carType == "Economy" || carType == "ComfortPlus")
            sql_query = "UPDATE cars SET validation = 'waiting' WHERE number = '" + driver.car.number + "';";
    ...
    void startRideStory() {
        ...
        try {
            if (driver.isBlocked == "true")
                throw driverIsBlockedException();
            if (driver.bussnesCar.number != "")
                driver.bussnesCar.parkRightInFrontOfTheEntrance();
            else if (driver.comfortCar.number != "")
                driver.comfortCar.usageBottleOfwater();
        ...
        catch (driverIsBlockedException e) {
        }
    }
    ...
    void getOrder() {
        ...
        string sql_query = "SELECT * FROM driver WHERE name = '" + driver.name + "' AND password = '" + driver.password + "';";
        selectData("WEndex_Taxi_BD.db", sql_query, 5);
        driver = tmpDriver;
        ...
        try 
        {
            if (driver.isBlocked == "true")
                throw driverIsBlockedException();
            if (driver.bussnesCar.number != "") {
                if (driver.bussnesCar.validation == "is not validated")
                    cout << "Car " << driver.bussnesCar.number << " not validated, you can not use it" << endl;
                else if (driver.bussnesCar.validation == "validated") {
                    sql_query = "SELECT * FROM orders WHERE carType = '" + driver.bussnesCar.carType + "' AND status = 'new';";
                    selectData("WEndex_Taxi_BD.db", sql_query, 3);
                    for (int i = 0; i < tmpHistory.size(); i++)
                    {
                        orders.push_back(tmpHistory[i]);
                    }
                    tmpHistory = vector<Ride>();
                }
            }
            if (driver.comfortCar.number != "") {
                if (driver.comfortCar.validation == "is not validated")
                    cout << "Car " << driver.comfortCar.number << " not validated, you can not use it" << endl;
                else if (driver.comfortCar.validation == "validated") {
            ...
            if (driver.car.number != "") {
                if (driver.car.validation == "is not validated")
                    cout << "Car " << driver.car.number << " not validated, you can not use it" << endl;
                else if (driver.car.validation == "validated") {
    ...
    void getOrderStory() {
        ...
        tmpBussnesCar = driver.bussnesCar;
        tmpComfortCar = driver.comfortCar;
        tmpCar = driver.car;
        ...
        driver.comfortCar = tmpComfortCar;
        driver.car = tmpCar;
        sql_query = "SELECT * FROM cars WHERE number = '" + driver.car.number + "';";
        selectData("WEndex_Taxi_BD.db", sql_query, 4);
        driver.car = tmpCar;
        sql_query = "SELECT * FROM cars WHERE number = '" + driver.bussnesCar.number + "';";
        selectData("WEndex_Taxi_BD.db", sql_query, 4);
        driver.bussnesCar = tmpBussnesCar;
        sql_query = "SELECT * FROM cars WHERE number = '" + driver.comfortCar.number + "';";
        selectData("WEndex_Taxi_BD.db", sql_query, 4);
        driver.comfortCar = tmpComfortCar;
        tmpBussnesCar = BussnessCar();
        tmpComfortCar = ComfortCar();
        ...
        try
        {
            ...
            sql_query = "";
            if (driver.bussnesCar.number != "") {
                if (driver.bussnesCar.validation == "not validated")
                    cout << "Car " << driver.bussnesCar.number << " not validated, you can not use it" << endl;
                else if (driver.bussnesCar.validation == "validated") {
                    sql_query = "SELECT * FROM orders WHERE carType = '" + driver.bussnesCar.carType + "' AND status = 'new';";
                    selectData("WEndex_Taxi_BD.db", sql_query, 3);
            ...
            if (driver.comfortCar.number != "") {
                if (driver.comfortCar.validation == "not validated")
                    cout << "Car " << driver.comfortCar.number << " not validated, you can not use it" << endl;
                else if (driver.comfortCar.validation == "validated") {
                    sql_query = "SELECT * FROM orders WHERE carType = '" + driver.comfortCar.carType + "' AND status = 'new';";
                    selectData("WEndex_Taxi_BD.db", sql_query, 3);
            ...
            if (driver.car.number != "") {
                if (driver.car.validation == "not validated")
                    cout << "Car " << driver.car.number << " not validated, you can not use it" << endl;
                else if (driver.car.validation == "validated") {
                    sql_query = "SELECT * FROM orders WHERE carType = '" + driver.car.carType + "' AND status = 'new';";
                    selectData("WEndex_Taxi_BD.db", sql_query, 3);
            ...
            if (orders.size() > 1) {
                ...
                sql_query = "UPDATE orders SET status = 'work' WHERE passenger_id = " +
                    to_string(orders[0].id) + " AND destination = '" + orders[0].destination + "';";
                updateData("WEndex_Taxi_BD.db", sql_query);
        ...
        if (code == 1)
        {
            ...
            while (code == 0)
            {
                ...
                case 3:
                    ...
                    cout << "Enter password:" << endl;
                    getline(cin, password);
                    adminSession.logIn(name, password);
                    code = 0;
                    while (code == 0) {
                        cout << endl << "Choose option:" << endl << endl;
                        cout << "[1] Block passenger " << endl;
                        cout << "[2] Unblock passenger" << endl;
                        cout << "[3] Block driver" << endl;
                        cout << "[4] Unblock driver" << endl;
                        cout << "[5] Validate cars" << endl;
                        cout << "[0] Exit" << endl;
                        cin >> code;
                        ...
                        {
                        case 1:
                            adminSession.blockPassenger();
                            code = 0;
                            break;
                        case 2:
                            adminSession.unblockPassenger();
                            code = 0;
                            break;
                        case 3:
                            adminSession.blockDriver();
                            code = 0;
                            break;
                        case 4:
                            adminSession.unblockDriver();
                            code = 0;
                            break;
                        case 5:
                            adminSession.validateCar();
                            code = 0;
                            break;
                        case 0:
                            cout << endl << "Bye..." << endl;
...
int selectData(const char* dir, string query, int code) {
    ...
    if (code == 1)
        ...
        sqlite3_exec(DB, query.c_str(), getAdmin, NULL, NULL);
    else if (code == 7)
        sqlite3_exec(DB, query.c_str(), getCars, NULL, NULL);

    return 0;
}

int getCars(void* NotUsed, int resultNum, char** values, char** colNames) {
    ...
    Car newCar(model, color, carType, number, validation);
    tmpCars.push_back(newCar);
...
int getAdmin(void* NotUsed, int resultNum, char** values, char** colNames) {
    ...
    Admin newAdmin(id, name, password);
    tmpAdmin = newAdmin;
...
int getCar(void* NotUsed, int resultNum, char** values, char** colNames) {
    ...
    if (carType == "Comfort") {
        ComfortCar newCar(model, color, carType, number, status, latitude, longitude, validation);
        tmpComfortCar = newCar;
    }
    else if (carType == "Business") {
        BussnessCar newCar(model, color, carType, number, status, latitude, longitude, validation);
        tmpBussnesCar = newCar;
    }
    else {
        Car newCar(model, color, carType, number, status, latitude, longitude, validation);
        tmpCar = newCar;
    }
    return 0;
}
...
int getUser(void* NotUsed, int resultNum, char** values, char** colNames) {
    ...
    Passenger newPass(name, address, payment1, payment2, password, history, rating, id, isBlocked);
...
int getDriver(void* NotUsed, int resultNum, char** values, char** colNames) {
    ...
    Driver newDriver(id, rating, name, password, history, status, isBlocked);
    tmpDriver = newDriver;
